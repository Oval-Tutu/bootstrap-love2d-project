...
uniform float gradientCurve = 1.3;  // Controls gradient curve steepness
uniform float powerFactor = 1.2;    // Controls 3D effect intensity
uniform float edgeSmoothing = 1.0;  // Controls anti-aliasing amount

vec4 effect(vec4 color, sampler2D texture, vec2 texture_coords, vec2 screen_coords) {
  // Calculate distance from current pixel to eye center (more efficiently)
  float distSq = dot(screen_coords - eyeCenter, screen_coords - eyeCenter);
